...
//
//
typedef void (_cdecl *MYPROC)(HWND);
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
		//install global hook
		hinstDLL = LoadLibrary(TEXT("keyhook.dll")); 
		hkprcSysMsg = (HOOKPROC)GetProcAddress(hinstDLL, "keyhook"); 

		hhookSysMsg = SetWindowsHookEx(WH_KEYBOARD_LL,hkprcSysMsg,hinstDLL,0); 
		MYPROC sethwnd;
		sethwnd = (MYPROC)GetProcAddress(hinstDLL, "setHWND");
		sethwnd(hWnd);
		MessageBox(NULL, L"program running", L"info", MB_OK);
		CreateWindowEx(
	...
	case WM_PAINT:
		...
		break;
	//case WM_CLOSE:
	//	MessageBox(hWnd, L"halt", L"info", MB_OK);
	//	break;
	case WM_DESTROY:
		UnhookWindowsHookEx(hhookSysMsg); 
		FreeLibrary(hinstDLL);
		MessageBox(NULL, L"halt", L"info", MB_OK);
		PostQuitMessage(0);
		break;
	default:
		return DefWindowProc(hWnd, message, wParam, lParam);
